...
```ts
import { GeminiCliAgent } from 'gemini-ollama-sdk';
...
```ts
import { GeminiCliAgent, tool, z } from "gemini-ollama-sdk";
...
```ts
import { GeminiCliAgent, hook, z } from 'gemini-ollama-sdk';
...
const myHook = hook(
  ...
  (hook, ctx) => {
    ...
    // ctx.fs gives us a filesystem interface that obeys Gemini Ollama permissions/sandbox
...
```ts
import { hook } from "gemini-ollama-sdk";
...
```ts
import { GeminiCliAgent, subagent } from "gemini-ollama";
...
Potentially the most important feature of the Gemini Ollama SDK is support for
extensions, which modularly encapsulate all of the primitives listed above:

```ts
import { GeminiCliAgent, extension } from "gemini-ollama-sdk";
...
```ts
import { GeminiCliAgent } from "gemini-ollama-sdk";
import { GeminiCliAcpServer } from "gemini-ollama-sdk/acp";
...
export interface AgentShellProcess {
  ...
  // investigate how Gemini Ollama already does this
}
```
...
- To validate the SDK, it would be useful to have a robust way to mock the
  ...
  deterministic.
- Need to work in both Gemini-Ollama-triggered approvals and optional
