...
for the Gemini Ollama. The ACP allows external clients (like IDE extensions) to
communicate with the Gemini Ollama agent over a structured JSON-RPC based protocol.
...
```bash
# General pattern
npm test -w gemini-ollama -- src/acp/<test-file-name>.ts

# Example
npm test -w gemini-ollama -- src/acp/acpRpcDispatcher.test.ts
```
...
```bash
source ~/.nvm/nvm.sh && nvm use default && npm test -w gemini-ollama -- src/acp/acpSession.test.ts
```
