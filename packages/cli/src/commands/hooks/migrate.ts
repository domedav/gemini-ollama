...
import { debugLogger, getErrorMessage } from 'gemini-ollama-core';
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Gemini Ollama',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: gemini hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Gemini Ollama format.',
      );
    }
    await exitCli();
  },
