...
to the Gemini Ollama Policy Engine.
...
1.  Link this extension to your local Gemini Ollama installation:

    ```bash
    gemini extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your Gemini Ollama session.
...
For security, Gemini Ollama ignores any `allow` decisions or `yolo` mode
