...
import type { Config, SkillDefinition } from 'gemini-ollama-core';
import {
  SettingScope,
  type LoadedSettings,
  ...
  describe('disable/enable', () => {
    ...
    it('should show error if skills are disabled by admin during disable', async () => {
      ...
      expect(context.ui.addItem).toHaveBeenCalledWith(
        expect.objectContaining({
          type: MessageType.ERROR,
          text: 'Agent skills is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-gemini-ollama',
        }),
        expect.any(Number),
      );
    });
