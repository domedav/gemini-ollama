  ...
  describe('save subcommand', () => {
    ...
    it('should save the conversation if overwrite is confirmed', async () => {
      ...
      expect(mockSaveCheckpoint).toHaveBeenCalledWith(
        { history, authType: AuthType.OLLAMA },
        tag,
      );
      expect(result).toEqual({
        type: 'message',
  ...
  describe('resume subcommand', () => {
    ...
    it('should block resuming a conversation with mismatched authType', async () => {
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'error',
        content: `Cannot resume chat. It was saved with a different authentication method (${AuthType.OLLAMA}) than the current one (${AuthType.OLLAMA}).`,
      });
    });
