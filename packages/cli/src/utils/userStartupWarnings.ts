  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    try {
      ...
      if (path.resolve(workspaceRealPath) === path.resolve(homeRealPath)) {
        ...
        return 'Warning you are running Gemini Ollama in your home directory.\nThis warning can be disabled in /settings';
      }
      return null;
    } catch {
...
const rootDirectoryCheck: WarningCheck = {
  ...
  check: async (workspaceRoot: string, _settings: Settings) => {
    try {
      ...
      const errorMessage =
        'Warning: You are running Gemini Ollama in the root directory. Your entire folder structure will be used for context. It is strongly recommended to run in a project-specific directory.';
  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    if (isHeadlessMode()) {
      throw new FatalUntrustedWorkspaceError(
        'Gemini Ollama is not running in a trusted directory. To proceed, either use `--skip-trust`, ' +
