...
import { isGitRepository, debugLogger } from 'gemini-ollama-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/opt/homebrew/Cellar/gemini-ollama/1.0.0/bin/gemini';
    process.argv[1] = cliPath;

    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew --prefix gemini-ollama')) {
        return '/opt/homebrew/opt/gemini-ollama';
      }
    ...
    mockedRealPathSync.mockImplementation((p) => {
      ...
      if (p === '/opt/homebrew/opt/gemini-ollama') {
        return '/opt/homebrew/Cellar/gemini-ollama/1.0.0';
      }
      return String(p);
    });
    ...
    expect(mockedExecSync).toHaveBeenCalledWith(
      expect.stringContaining('brew --prefix gemini-ollama'),
      expect.anything(),
    );
    expect(info.packageManager).toBe(PackageManager.HOMEBREW);
    expect(info.isGlobal).toBe(true);
    expect(info.updateMessage).toBe(
      'Installed via Homebrew. Please update with "brew upgrade gemini-ollama".',
    );
  });
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/gemini-ollama/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g gemini-ollama@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/gemini-ollama/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add gemini-ollama@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/install/global/node_modules/gemini-ollama/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g gemini-ollama@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g gemini-ollama@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect Volta installation (Unix-style)', () => {
    const voltaPath =
      '/Users/test/.volta/tools/image/node/20.0.0/lib/node_modules/gemini-ollama/dist/index.js';
    ...
    expect(info.updateCommand).toBe('volta install gemini-ollama@latest');
  });

  it('should detect Volta installation (Windows-style)', () => {
    const voltaPath =
      'C:\\Users\\test\\AppData\\Local\\Volta\\tools\\image\\node\\20.0.0\\node_modules\\gemini-ollama\\dist\\index.js';
    process.argv[1] = voltaPath;
    mockedRealPathSync.mockReturnValue(voltaPath);
  ...
  it('should NOT detect Homebrew if gemini-ollama is installed in brew but running from npm location', () => {
    Object.defineProperty(process, 'platform', {
      value: 'darwin',
    });
    // Path looks like standard global NPM
    const cliPath =
      '/usr/local/lib/node_modules/gemini-ollama/dist/index.js';
    process.argv[1] = cliPath;

    // Setup mocks
    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew list')) {
        return Buffer.from('gemini-ollama\n');
      }
