...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-gemini-ollama/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-gemini-ollama release:`,
      error,
    );
    throw new Error(
      `Unable to determine the latest run-gemini-ollama release on GitHub.`,
    );
  }
};
