...
function checkRelease() {
  try {
    ...
    const bucketUri = 'gs://gemini-ollama-vscode-extension/release/1p/signed';
    ...
    const vsixFiles = files.filter((file) =>
      /signed-gemini-ollama-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(
        file,
      ),
    );
    ...
    const match =
      /signed-gemini-ollama-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(
        fileName,
      );
