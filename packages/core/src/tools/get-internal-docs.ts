...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Gemini Ollama documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Gemini Ollama documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Gemini Ollama's internal documentation.
