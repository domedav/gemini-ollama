...
describe('modifyWithEditor', () => {
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/gemini-ollama-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/gemini-ollama-modify-test-file-new-\d+\.txt$/);

    const diffDirPrefix = path.join(os.tmpdir(), 'gemini-ollama-tool-modify-');
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/gemini-ollama-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/gemini-ollama-modify-test-file-new-\d+$/);
