...
export function validateWorkspacePath(
  ...
  if (!isWithinWorkspace) {
    return {
      isValid: false,
      error: `Directory mismatch. Gemini Ollama is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
        ', ',
      )}`,
    };
  }
