  ...
  async connect(options: { logToConsole?: boolean } = {}): Promise<void> {
    ...
    if (!this.currentIde) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run Gemini Ollama in one of these supported IDEs: Antigravity, VS Code, or VS Code forks.`,
        false,
      );
      return;
    }
