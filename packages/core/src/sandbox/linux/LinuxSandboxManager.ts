...
function getSeccompBpfPath(): string {
  ...
  const tempDir = fs.mkdtempSync(join(os.tmpdir(), 'gemini-ollama-seccomp-'));
...
export class LinuxSandboxManager implements SandboxManager {
  ...
  private getMaskFilePath(): string {
    ...
    const tempDir = fs.mkdtempSync(join(os.tmpdir(), 'gemini-ollama-mask-file-'));
  ...
  private writeArgsToTempFile(args: string[]): string {
    const tempFile = join(
      os.tmpdir(),
      `gemini-ollama-bwrap-args-${Date.now()}-${Math.random().toString(36).slice(2)}.args`,
    );
