...
describe('MacOsSandboxManager', () => {
  ...
  beforeEach(() => {
    mockWorkspace = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), 'gemini-ollama-macos-test-')),
    );

    const allowedPathTemp = path.join(
      os.tmpdir(),
      'gemini-ollama-macos-test-allowed-' + Math.random().toString(36).slice(2),
    );
  ...
  describe('prepareCommand', () => {
    it('should correctly format the base command and args', async () => {
      ...
      expect(result.args[1]).toMatch(/gemini-ollama-seatbelt-.*\.sb$/);
