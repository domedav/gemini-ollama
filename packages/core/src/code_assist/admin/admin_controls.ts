...
export function getAdminErrorMessage(
  ...
  return `${featureName} is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-gemini-ollama${projectParam}`;
}
...
export function getAdminBlockedMcpServersMessage(
  ...
  return `${count} MCP ${serverText} not allowlisted by your administrator. To enable ${
    count === 1 ? 'it' : 'them'
  }, please request an update to the settings at: https://goo.gle/manage-gemini-ollama${projectParam}`;
}
