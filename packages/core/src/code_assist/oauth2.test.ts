  ...
  describe('with encrypted flag false', () => {
    ...
    beforeEach(() => {
      ...
      tempHomeDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'gemini-ollama-test-home-'),
      );
    ...
    it('should emit post_auth event when loading cached credentials', async () => {
      ...
      await getOauthClient(AuthType.OLLAMA, mockConfig);
      await eventPromise;
    });
    ...
    it('should throw FatalAuthenticationError in non-interactive session when manual auth is required', async () => {
      ...
      await expect(
        getOauthClient(AuthType.OLLAMA, mockConfigNonInteractive),
    ...
    it('should cache Google Account when logging in with user code', async () => {
      ...
      await getOauthClient(AuthType.OLLAMA, mockConfigWithNoBrowser);
    ...
    describe('in Cloud Shell', () => {
      ...
      it('should return the Compute client on successful ADC authentication', async () => {
        const client = await getOauthClient(AuthType.OLLAMA, mockConfig);
      ...
      it('should throw an error if ADC fails', async () => {
        ...
        await expect(
          getOauthClient(AuthType.OLLAMA, mockConfig),
        ).rejects.toThrow(
    ...
    describe('error handling', () => {
      ...
      it('should handle user code authentication failure with descriptive error', async () => {
        ...
        await expect(
          getOauthClient(AuthType.OLLAMA, mockConfigWithNoBrowser),
