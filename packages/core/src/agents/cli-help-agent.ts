...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about Gemini Ollama.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about Gemini Ollama itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized agent for answering questions about the Gemini Ollama application. Invoke this agent for questions regarding CLI features, configuration schemas (e.g., policies), or instructions on how to create custom subagents. It queries internal documentation to provide accurate usage guidance.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about Gemini Ollama.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about Gemini Ollama:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on Gemini Ollama. Your purpose is to provide accurate information about Gemini Ollama's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
