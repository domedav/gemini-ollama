...
17. **Policy Compliance (GCLI Classification)**: If a script utilizes Gemini Ollama
    for classification, ensure it does NOT use the specialized
    `tools/gemini-ollama-bot/ci-policy.toml`. It must rely on default or workspace
...
- **Update Structured Memory**: You MUST record your decision and reasoning in
  `tools/gemini-ollama-bot/lessons-learned.md` using the **Structured Markdown**
