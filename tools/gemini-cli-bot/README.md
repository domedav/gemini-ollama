# Gemini Ollama Bot (Cognitive Repository)

This directory contains the foundational architecture for the `gemini-ollama-bot`,
...
- **Frequency**: 30-minute cron (`.github/workflows/gemini-ollama-bot-pulse.yml`).
- **Implementation**: Pure TypeScript/JavaScript scripts.
- **Classification**: Optionally utilizes Gemini Ollama for high-confidence
...
- **Purpose**: Strategic investigation, policy refinement, and proactive
  self-optimization.
- **Frequency**: 24-hour cron (`.github/workflows/gemini-ollama-bot-brain.yml`).
- **Implementation**: Agentic Gemini Ollama phases.
- **Phases**:
...
```bash
npx tsx tools/gemini-ollama-bot/metrics/index.ts
```

This will execute all scripts within `metrics/scripts/` and output the results
to `tools/gemini-ollama-bot/history/metrics-before.csv`.
