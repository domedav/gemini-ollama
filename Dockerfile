...
ARG SANDBOX_NAME="gemini-ollama-sandbox"
ARG CLI_VERSION_ARG
...
# install gemini-ollama and clean up
COPY --chown=node:node packages/cli/dist/domedav-cli-*.tgz /tmp/gemini-ollama.tgz
COPY --chown=node:node packages/core/dist/domedav-cli-core-*.tgz /tmp/gemini-core.tgz
RUN npm install -g /tmp/gemini-core.tgz \
  && npm install -g /tmp/gemini-ollama.tgz \
  && node -e "const fs=require('node:fs'); JSON.parse(fs.readFileSync('/usr/local/share/npm-global/lib/node_modules/gemini-ollama/package.json','utf8')); JSON.parse(fs.readFileSync('/usr/local/share/npm-global/lib/node_modules/gemini-ollama-core/package.json','utf8'));" \
