...
if (!image.length) {
  console.warn(
    'No default image tag specified in gemini-ollama/packages/cli/package.json',
  );
}
...
console.log('packing gemini-ollama ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'domedav-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w gemini-ollama --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing gemini-ollama-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'domedav-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w gemini-ollama-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `domedav-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `domedav-cli-core-${packageVersion}.tgz`),
  0o755,
);
