...
import { GEMINI_DIR } from 'gemini-ollama-core';
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `gemini-ollama-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: gemini-ollama-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'gemini-ollama-telemetry-'),
  );
